...
class BuildRunMiniWeatherTest(rfm.RegressionTest):
    ...
    @run_before('compile')
    def setflags(self):
        self.build_system.config_opts=['-DCMAKE_Fortran_COMPILER=ftn -DLDFLAGS="-L${CRAY_PARALLEL_NETCDF_DIR}/lib -lpnetcdf" -DNX=400 -DNZ=200 -DSIM_TIME=100 -DOUT_FREQ=1000' ]
        if self.current_environ.name.startswith('PrgEnv-cray'):
            self.build_system.config_opts += ['-DFFLAGS="-I${CRAY_PARALLEL_NETCDF_DIR}/include" -DOPENMP_FLAGS="-homp"' ]
        elif self.current_environ.name.startswith('PrgEnv-gnu'):
            self.build_system.config_opts += ['-DFFLAGS="-I${CRAY_PARALLEL_NETCDF_DIR}/include -ffree-line-length-none"  -DOPENMP_FLAGS="-fopenmp"' ]
        elif self.current_environ.name.startswith('PrgEnv-aocc'):
            self.build_system.config_opts += ['-DFFLAGS="-I${CRAY_PARALLEL_NETCDF_DIR}/include -march=znver2" -DOPENMP_FLAGS="-fopenmp"'  ]
